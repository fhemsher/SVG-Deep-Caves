...
function editIconStart(evt)
{
    ...
    var myZoomLevel=+evt.target .getAttribute("InitZoom")
        ...
        for(var k = 0; k<cw.IconUnicode.length; k++)
        {
            ...
            if(myCode==unicode)
            {
                ...
                ActiveElem.attr("InitZoom", MapZoom)
                      ActiveElem.attr("onmouseover", "myZoomLevel("+MapZoom+","+("icon"+utcMS)+")")

         ActiveElem.attr("onmouseout", "removeZoomLevel()")
          setLatLng(document.getElementById("icon"+utcMS)) //---helperFuncts.js---
